...
class _NewsScreenState extends State<NewsScreen> {
  ...
  Future<void> _deleteNews(String key) async {
    try {
      final response = await http.delete(
        Uri.parse(
            'https://barr-502d7-default-rtdb.asia-southeast1.firebasedatabase.app/news/$key.json'),
      );

      if (response.statusCode == 200) {
        setState(() {
          _newsList.removeWhere((news) => news['key'] == key);
        });
      } else {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: _newsList.isEmpty
            ? Center(child: Text('Upload News', style: TextStyle(fontSize: 20)))
            : ListView.builder(
